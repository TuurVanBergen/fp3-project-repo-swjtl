...
async function fetchPersons(url) {
	try {
		...
		searchBar.addEventListener("input", function () {
			...
			const filteredPersons = arrPersons.filter(
				(item) =>
			...
			const filteredEquipment = arrEquipment.filter(
				(item) =>
					(item.itemLabel &&
						item.itemLabel.toLowerCase().includes(searchTerm)) ||
					(item.start && item.start.includes(searchTerm))
			);

			const filteredEvents = arrEvents.filter(
				(item) =>
					(item.itemLabel &&
						item.itemLabel.toLowerCase().includes(searchTerm)) ||
					(item.timeline_date &&
						item.timeline_date.slice(0, 4).includes(searchTerm))
			);

			const filteredTheatres = arrTheatres.filter(
				(item) =>
					(item.itemLabel &&
						item.itemLabel.toLowerCase().includes(searchTerm)) ||
